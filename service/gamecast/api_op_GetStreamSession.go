...
// Example CLI input aws gamecast get-stream-session --identifier
...
func streamSessionActiveStateRetryable(ctx context.Context, input *GetStreamSessionInput, output *GetStreamSessionOutput, err error) (bool, error) {
	...
	if err == nil {
		...
		value, ok := pathValue.(types.StreamSessionStatus)
		if !ok {
			return false, fmt.Errorf("waiter comparator expected types.StreamSessionStatus value, got %T", pathValue)
		}
